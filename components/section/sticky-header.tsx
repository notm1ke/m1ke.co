...
export function StickySectionHeader({ id, title, amount, className, sectionRef }: StickyHeaderProps) {
	...
	return (
		...
		>
			<h2 onClick={handleClick} className="text-2xl font-semibold text-purple-400 font-pixel-square cursor-pointer">
				{title}
			</h2>
			<Separator className="flex-1 bg-purple-500/40" />
			{amount && <h4 className="text-xl font-semibold text-purple-100 font-pixel-square">{amount}</h4>}
		</motion.div>
	);
}
